// 工作量证明机制的核心在于不断hash区块自身，将hash值与根据难度值计算出的一串大数对比，
// 如果自身hash小于大数则说明挖矿成功，否则变化自身随机数重新计算。
// 并且程序会随着出块间隔时间动态调节难度值（比如比特币）
package main
...
// 使用math/big包，根据全局变量的难度值diffNum计算出用于实际比较的一串大数newBigint
// 并同时将区块hash转换为大数hashInt 两个大数进行数值比较，如果hashInt小于newBigint 则代表挖矿成功
...
func main() {
	...
	for i := 0; i < 10; i++ {
		newBlock := mine("Good Luck" + strconv.Itoa(i))
